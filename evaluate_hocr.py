...
# def get_iter_text(parent):
# 	out = [y.text for y in parent.iterchildren()]
...
def get_adj_by_text(text, direction = 'default', strict = False, ymax= 99999, id = None):
	...
	cumulative_output=[]
	for occur_n,o in enumerate(all_occur):
		if direction == 'below':
...
if __name__ == "__main__":
	...
	jpg_path = 'invoice2/jpg/'+args.name+'.jpg'
	hocr_path = 'invoice2/hocr/'+args.name+'.hocr'
	...
	print(*table_using_lines,sep="\n")
	# print(*table_using_ranges,sep="\n")
	...
	writer = pd.ExcelWriter('invoice2/output/'+str(args.name)+'.xlsx')
